...
import rolesjson from '../../constants/roles'
...
export default class RBAC {
   ...
   * @static
   */
  static getPermissionNames(permissions: any, delimiter: string) {
    if (!delimiter) {
      throw new Error('Delimiter is not defined');
    }

    return permissions.map(
      (permission: any) => Permission.createName(permission[0], permission[1], delimiter),
    );
  }
  ...
  async init() {
    const { permissions, grants } = this.options;
    const roles = await rolesjson.defaultRoles.map((a) => a.id);
   ...
   */
  async grant(role: Role, child: Base) {
    if (!role || !child) {
   ...
   * @param {Base} child Instance of the role or permission
   */
  async revoke(role: Role, child: Base) {
    if (!role || !child) {
      throw new Error('One of item is undefined');
    }
   ...
   */
  async revokeByName(roleName: string, childName: string) {
   ...
   * @param {String} childName Instance of the role or permission
   */
  async grantByName(roleName: string, childName: string) {
    const [role, child] = await Promise.all([
      this.get(roleName),
      this.get(childName),
    ]);
   ...
   */
  async createPermission(action:string, resource: string) {
   ...
   * @param {String} resource Name of resource
   */
  async existsPermission(action: string, resource: string) {
  ...
  async getPermissionByName(name: string) {
    const data: any = Permission.decodeName(name, this.options.delimiter);
   ...
   */
  async createPermissions(resources: any) {
    ...
    await Promise.all(Object.keys(resources).map(async (resource: any) => {
      const actions = resources[resource];

      await Promise.all(actions.map(async (action: any) => {
        const permission = await this.createPermission(action, resource);
   ...
   * @param {Boolean} [add=true] True if you need to add it to the storage
   */
  async createRoles(roleNames: string[]) {
    const roles: any = {};
  ...
  async grants(roles: any) {
    ...
    await Promise.all(Object.keys(roles).map(async (roleName) => {
      ...
      await Promise.all(grants.map(async (grant: any) => {
   ...
   */
  async create(roleNames: any[], permissionNames: any[], grantsData: any) {
   ...
   * @private
   */
  async traverseGrants(roleName: any, cb: Function, next: any[] = [roleName], used: any = {}): Promise<any> {
   ...
   * @return {boolean}
   */
  async can(roleName: string, action: string, resource: string) {
    const can = await this.traverseGrants(roleName, (item: any) => {
   ...
   * @return {boolean}
   */
  async canAny(roleName: string, permissions: Object[]) {
   ...
   */
  async canAll(roleName: string, permissions: any[]) {
    // prepare the names of permissions
    const permissionNames = RBAC.getPermissionNames(permissions, this.options.delimiter);
    const founded: any = {};
    let foundedCount = 0;
    ...
    // traverse hierarchy
    await this.traverseGrants(roleName, (item: any) => {
   ...
   * @return {boolean}
   */
  async hasRole(roleName: string, roleChildName: string) {
    if (roleName === roleChildName) {
      return true;
    }

    const has = await this.traverseGrants(roleName, (item: any) => {
   ...
   * @return {string[]}
   */
  async getScope(roleName: string) {
    const scope: any[] = [];
    // traverse hierarchy
    await this.traverseGrants(roleName, (item:any) => {
