...
export default class Memory extends Storage {
  ...
  async remove(item: Base) {
    ...
    Object.keys(items).forEach((itemName: string) => {
      ...
      items[itemName].grants = grants.filter((grant: any) => grant !== name);
    });
  ...
  async revoke(role: Role, child: Base) {
    ...
    this.items[name].grants = grants.filter((grant:any) => grant !== childName);
  ...
  async getPermissions() {
    return this.items
      .reduce((filtered: Permission[], item: any) => {
        const { instance } = item;
